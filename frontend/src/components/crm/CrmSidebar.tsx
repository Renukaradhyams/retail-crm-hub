...
const GROUPS: { label: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: "/app/tv", label: "Live TV Display", icon: Tv, page: "tv" },
      { to: "/app/greeter", label: "Greeter Portal", icon: HandHeart, page: "greeter" },
...
export function CrmSidebar({
  ...
  useEffect(() => {
    async function fetchDivertsCount() {
      try {
        const { data } = await api.get("/crm/dashboard");
        setOpenDiverts(data.openDiverts || 0);
      } catch (err) {
        console.error("Failed to fetch open diverts count", err);
      }
    }
    fetchDivertsCount();
    const interval = setInterval(fetchDivertsCount, 30000);
